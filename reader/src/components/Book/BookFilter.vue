<template>
  <div>
    ...
    <div class="mb-4">
      <div
        class="relative w-full rounded-lg bg-white dark:bg-gray-800 shadow-sm overflow-hidden focus-within:ring-2 focus-within:ring-primary"
      >
        <input
          v-model="filters.q"
          type="text"
    ...
    <div v-if="hasAnyAdvanced" :class="[
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <button
          ...
          class="px-3 py-1.5 rounded-full text-sm bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors appearance-none border border-transparent focus:outline-none shadow-sm"
        >
          {{ ui.expanded ? '收起' : '展开' }}
        </button>
      </div>
      ...
      <!-- 书架（单选） -->
      <div v-if="showShelves && shelves.length > 0" class="mb-4">
      ...
      <div v-show="!enableExpand || ui.expanded">
        <!-- 标签（多选） -->
        <div v-if="showTags && tags.length > 0" class="mb-4">
          <div class="text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">标签</div>
          <div class="flex flex-wrap gap-2">
      ...
      <div class="mt-4 flex justify-end gap-2">
        <div class="flex-grow text-center">
          <el-button size="default" class="rounded-full px-8" @click="onReset">重置</el-button>
          <el-button type="primary" size="default" class="rounded-full px-8" @click="onSearch">
            搜索
          </el-button>
        </div>
      </div>
