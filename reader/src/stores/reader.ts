  ...
  const loadChapters = async () => {
    ...
    try {
      ...
      if (response.data.book) {
        // API返回的book是简化版本，我们暂时只保存基本信息
        bookId.value = response.data.book.id
        // TODO: 后续可以通过单独的API获取完整的Book信息
      }

      chapters.value = response.data.chapters.map((chapter: Chapter, index: number) => ({
        ...chapter,
        index
      }))
