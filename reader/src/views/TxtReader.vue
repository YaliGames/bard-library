...
import type { ReaderContext, SearchResult } from '@/types/reader'
  ...
  try {
    ...
    if (typeof initialChapterIndex.value === 'number') {
      ...
      try {
        let p
        try {
          p = await progressApi.get(bookId.value, fileId)
        } catch (e) {
          console.warn('Failed to load progress (offline?):', e)
        }
        if (!p) throw new Error('No progress data')
...
async function jumpToBookmark(b: Bookmark) {
  try {
    ...
    // 优先使用后端提供的绝对偏移（absStart/absEnd）进行精确定�?
    if (
      ...
      // 若跳转到非当前章节，先设�?pendingScroll（包含章节局部位置），并加载目标章节
...
function onKeyDown(e: KeyboardEvent) {
  ...
  // Ctrl+Shift+F �?Cmd+Shift+F：打开全文搜索
...
// 提供一�?readerContext 给子组件，子组件可以通过 inject('readerContext') 访问页面级状态与方法
const readerContext = {
  // 基本数据
  chapters,
  currentChapterIndex,
  ...
  hasPrevChapter,
  hasNextChapter,
  // autoScrollCategory removed - settings中没有txtReader配置
