<template>
  <el-tabs v-model="innerTab" stretch class="!mb-1">
        ...
        >
          <span
            :class="[
              ...
              currentChapterIndex === c.index ? 'text-primary' : '',
            ]"
          >
            #{{ c.index }}
          </span>
    ...
    <el-tab-pane label="书签" name="bookmarks_old" v-if="false">
      ...
      <div v-else-if="filteredBookmarks.length === 0" class="text-gray-500 text-sm">
        当前文件暂无书签
      </div>
      <div class="max-h-[60vh] overflow-auto flex flex-col gap-1">
        <div
          ...
          :key="b.id"
          class="group flex items-start gap-2 p-2 rounded-md border hover:bg-gray-50"
        >
          <button
            @click="emit('jump', b)"
            class="text-xs px-2 py-1 rounded bg-primary/10 text-primary hover:bg-primary/15"
          >
            跳转
          </button>
  ...
  try {
    ...
    if (typeof loc?.absStart === 'number' && props.chapters.length > 0) {
      ...
      for (let i = 0; i < props.chapters.length; i++) {
        ...
        if (abs >= (ch as any).offset && abs < (ch as any).offset + (ch as any).length) {
          idx = i
          break
        }
      }
