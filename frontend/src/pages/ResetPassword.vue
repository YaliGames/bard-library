          ...
          <div class="w-full flex-1 mt-8">
            <div v-if="canRecover" class="mx-auto max-w-xs">
              ...
              <input
                ...
                type="text" v-model="token" placeholder="请输入邮件中的重置令牌" />
              <input
                class="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white mt-4"
                type="password" v-model="password" placeholder="请输入新密码" />
              <input
                class="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white mt-4"
                type="password" v-model="password2" placeholder="请再次输入新密码" />
...
async function onSubmit(){
  ...
  if (!e) { err.value = '请输入邮箱'; return }
  const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRe.test(e)) { err.value = '邮箱格式不正确'; return }
  if (!token.value) { err.value = '请输入重置令牌'; return }
  if (!password.value) { err.value = '请输入新密码'; return }
