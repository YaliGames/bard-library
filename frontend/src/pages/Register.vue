          ...
          <div class="w-full flex-1 mt-8">
            <div class="mx-auto max-w-xs">
              ...
              <input
                ...
                type="email" v-model="form.email" placeholder="请输入邮箱" />
              <input
                class="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white mt-4"
                type="password" v-model="form.password" placeholder="请输入密码" />
              <input
                class="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white mt-4"
                type="password" v-model="form.password2" placeholder="请再次输入密码" />
            ...
            <div class="flex flex-col items-center">
              ...
              <router-link :to="{ name: 'forgot' }" v-if="false"
...
function validateForm(): boolean {
  ...
  if (!name) { err.value = '请输入昵称'; return false }
  if (name.length < 2 || name.length > 20) { err.value = '长度 2-20 个字符'; return false }

  const email = (form.email || '').trim()
  if (!email) { err.value = '请输入邮箱'; return false }
  const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRe.test(email)) { err.value = '邮箱格式不正确'; return false }

  if (!form.password) { err.value = '请输入密码'; return false }
  if (form.password.length < 6) { err.value = '至少 6 位'; return false }

  if (!form.password2) { err.value = '请再次输入密码'; return false }
  if (form.password !== form.password2) { err.value = '两次输入的密码不一致'; return false }
