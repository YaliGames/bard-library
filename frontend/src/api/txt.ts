...
export interface Chapter { index:number; title?:string|null; offset:number; length:number; file_id?: number; book_id?: number }
...
export const txtApi = {
  ...
  getChapterContent: (fileId: number, index: number) => {
    return http.get<{ content: string; book_id?: number; file_id?: number; index: number; title?: string | null }>(`/api/v1/txt/${fileId}/chapters/${index}`);
  },
